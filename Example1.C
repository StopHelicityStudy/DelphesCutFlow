...
root -l Example1.C'("stop_a/stop_a/stop_a_1.root")' -b
*/
...
void Example1(const char *inputFile)
{
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
  {
    ...
    std::cout << "new event\n" << std::endl;
    std::cout << branchElectron->GetEntries() << std::endl;
    ...
    std::cout << muon->P4().Pt() << std::endl;


    std::cout << branchJet->GetEntries() << std::endl;
    ...
    if(branchJet->GetEntries() > 0)
    {
      ...
      for(int i=0; i < branchJet->GetEntries(); i++){
        ...
        std::cout << "\t" << muon->P4().DeltaR(lepJet->P4())  << " " << lepJet->BTag<< std::endl;
